...
class User:
    def __init__(self, userId: str):
        ...
        self.__index = Path(f"{self.__user}/corpus.index")
        self.__tsne = Path(f"{self.__user}/tsne.npy")
        self.__settings = Path(f"{self.__user}/settings.json")
        self.__clusterer = Path(f"{self.__user}/doc_clusterer.bin")

        if not os.path.isdir(self.__user):
            return None
        else:
            Path(self.__sessions).mkdir(parents=True, exist_ok=True)
            Path(self.__corpus).mkdir(parents=True, exist_ok=True)
    ...
    def append_session(
        ...
        session_path = Path(f"{self.__sessions}/{id}.json")
