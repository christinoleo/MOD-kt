...
load_dotenv("../web/.env")
print(os.getenv("USER_PATH", "./users"))
print(os.listdir(os.getenv("USER_PATH", "./users")))

logging.basicConfig(
    filename=os.path.abspath("./log/server.log") if os.getenv("LOGGING") else None,
    level=logging.DEBUG,
...
NLTK_Downloader(
    ["stopwords", "wordnet", "punkt", "averaged_perceptron_tagger"], quiet=True
)
...
def sanity_check() -> bool:
    ...
    try:
        for model in m_type.keys():
            ...
            models = [
                m_file
                for m_file in os.listdir(m_type_dir)
            ...
            for m_file in models:
                spec = spec_from_file_location(
                    os.path.splitext(m_file)[0], f"{m_type_dir}/{m_file}"
                )
