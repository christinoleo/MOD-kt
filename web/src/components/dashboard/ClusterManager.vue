    ...
    <b-container fluid id="clusterManagerWrapper">
      <b-row cols="4" class="p-1">
        ...
        >
          <b-card
            no-body
            class="cluster-card text-center"
            :id="'cluster_card_' + index"
            text-variant="white"
            :class="[
              'cluster-card',
              'text-center',
              cluster == highlight ? 'highlighted' : '',
            ]"
      ...
      <div class="d-block text-center">
        <b-input-group>
          <b-form-input
            ...
            :state="validClusterName(modal.cluster_name)"
          ></b-form-input>
          <b-form-input
            class="mb-1"
            v-model="modal.color"
            ...
            <small
              ><b-button
                ...
                :disabled="
                  ...
                  validClusterName(modal.cluster_name)
                "
            ...
            >
              ...
              <small>
                <b-button-group size="sm">
                  <b-button variant="outline-danger" @click="word.weight = -1">
                  ...
                  <b-button variant="outline-success" @click="word.weight = 1">
...
export default {
  ...
  methods: {
    validClusterName(cluster_name) {
      ...
      return (
        modal.cluster_name?.length > 0 &&
        ((modal.index < this.clusters.cluster_k &&
          !this.clusters.cluster_names
            .filter((d, i) => i != modal.index)
    ...
    submitChanges() {
      ...
      d3.select(`#cluster_card_${this.modal.index} header`).style(
        "background",
        this.modal.color
      );
    ...
    callDeleteCluster() {
      if (
        confirm(
          `You're about to delete cluster ${this.modal.cluster_name}, are you sure?`
        )
      ) {
