<template>
  <b-card no-body footer-tag="footer">
    <div id="projectionSwitcher" class="d-flex w-100 justify-content-between">
      <b-form-radio-group
        v-model="projection"
      ...
      <b-button-group size="sm" class="ml-auto mr-2">
        <b-button
          ...
          @click="silhouette_score = callSilhouette()"
          >Silhouette</b-button
        >
        ...
        <b-row h-align="center">
          ...
          <b-col class="w-50">
            <template v-if="projection == 't-SNE'">
              <b-input-group prepend="Perplexity" size="sm">
                <b-form-input
                  class="col-9"
                  v-model="perplexity"
                  size="sm"
                ...
                ></b-form-input>
                <b-input-group-append>
                  <b-button @click="requestProjection('t-SNE')" variant="info">
            ...
            <template v-else>
              <b-row>
                <b-input-group prepend="Charge" size="sm">
                  <b-form-input
                    size="sm"
                    v-model="charge"
                    type="range"
              ...
              <b-row>
                <b-input-group prepend="Link distance" size="sm">
                  <b-form-input
                    size="sm"
...
export default {
      ...
      handler() {
        ...
        if (this.highlight == "") {
          ...
          this.link.classed(
            "faded",
            (d) =>
              !(doc_ids.includes(d.source.id) && doc_ids.includes(d.target.id))
          );
    ...
    selected: {
      ...
      handler() {
        ...
        if (this.selected.length == 0) {
          this.node.classed("selected", false).classed("faded", false);
          this.link.classed("selected", false).classed("faded", false);
        } else {
  ...
  computed: {
    projection: {
      ...
      set(projection) {
        this.$store.commit("session/setProjection", projection);
      },
    },
    link_selector: {
      get() {
        return this.controls.link_selector;
      },
      set(link_selector) {
    ...
    n_neighbors: {
      get() {
        return this.controls.n_neighbors;
      },
      set(n_neighbors) {
    ...
    perplexity: {
      get() {
        return this.controls.tsne.perplexity;
      },
      set(perplexity) {
        this.$store.commit("session/setPerplexity", perplexity);
      },
    },
    charge: {
      get() {
    ...
    this.svg = d3
      ...
      .call(
        d3
          ...
          .scaleExtent([0.1, 8])
          .on("zoom", (e) => {
            objRef.canvas.attr("transform", e.transform);
          })
      );
  ...
  methods: {
    ...
    updateLayout() {
      ...
      this.node = this.node
        ...
        .attr("cx", (d) => d.x)
        .attr("cy", (d) => d.y)
        .classed("selected", (d) => objRef.selected.includes(d.id))
        .classed("new_doc", (d) => objRef.new_docs.includes(d.id))
        .attr("fill", (d, i) => {
        ...
        .on("mousemove", (event, node) => {
          ...
          Object.keys(doc.term_frequency)
            .sort(
              (a, b) => doc.term_frequency[`${b}`] - doc.term_frequency[`${a}`]
            )
            .slice(0, 5)
